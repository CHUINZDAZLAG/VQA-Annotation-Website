...
export default function ReviewWorkspace() {
  ...
  async function saveReview() {
    ...
    try {
      const saved = await authService.reviewSlide(taskId, selectedSlide.annotation_id, { decision, reject_reason: decision === 0 ? rejectReason : null });
      setSlides((current) => current.map((slide) => slide.annotation_id === selectedSlide.annotation_id ? { ...slide, annotation: saved } : slide));
  ...
  return <UserShell eyebrow="Reviewer" title="Review Workspace" subtitle={`Task #${taskId}`} actions={<Link className="admin-action admin-action-secondary" to={`/tasks/${taskId}`}>← &nbsp;Back to task</Link>}>
    ...
    {!slides.length ? <section className="admin-section">No processed slides are available for review.</section> : <div className="annotation-layout">
      <aside className="annotation-slide-list"><h2 className="admin-section-title">Slides</h2>{slides.map((slide, index) => <button className={`annotation-slide-item ${index === selectedIndex ? 'selected' : ''}`} key={slide.annotation_id} onClick={() => setSelectedIndex(index)} type="button"><span className="annotation-slide-check">{slide.annotation?.reviewer_label === 1 ? '✓' : slide.annotation?.reviewer_label === 0 ? '×' : '○'}</span><span>Page {String(slide.page_number).padStart(2, '0')}</span><small>{slide.image_id}</small></button>)}</aside>
