...
export default function AdminTaskResults() {
  ...
  async function load() {
    try {
      ...
      setDriveFolderId(taskValue.admin_drive_folder_id || '');
    } catch (requestError) {
      ...
      <div className="admin-results">
        ...
        <section className="mt-8 border border-slate-700 bg-slate-900 p-5">
          ...
          <dl className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div><dt className="text-xs text-slate-400">Admin Default Folder</dt><dd>{task?.admin_drive_folder_id || 'Not configured'}</dd></div>
            <div><dt className="text-xs text-slate-400">Main Override Folder</dt><dd>{task?.annotator_drive_folder_id || 'Not configured'}</dd></div>
            <div><dt className="text-xs text-slate-400">Effective Destination</dt><dd>{task?.drive_folder_id || 'Not configured'}</dd></div>
        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-2"><form className="border border-slate-700 bg-slate-900 p-4" onSubmit={configureDrive}><h2 className="text-lg font-semibold">Default Google Drive folder</h2><p className="mt-2 text-sm text-slate-400">Main can override this destination for the task. Credentials remain backend-only.</p><div className="mt-4 flex gap-2"><input className="flex-1 bg-slate-800 p-2" placeholder="Folder ID or Google Drive folder URL" value={driveFolderId} onChange={(event) => setDriveFolderId(event.target.value)} /><button className="bg-white px-3 py-2 text-slate-950" type="submit">Save folder</button></div></form><div className="border border-slate-700 bg-slate-900 p-4"><h2 className="text-lg font-semibold">Export history</h2>{exports.length === 0 && <p className="mt-2 text-sm text-slate-400">No final export yet.</p>}{exports.map((item) => <div className="mt-3 border-t border-slate-800 pt-3 text-sm" key={item.id}><p><button className="text-cyan-300 underline" onClick={() => authService.downloadTaskExport(taskId, item.id)} type="button">{item.file_name}</button> <span className="text-slate-400">{item.format} · {item.status}</span></p><p className="mt-1 text-xs text-slate-500">{new Date(item.exported_at).toLocaleString()}</p>{item.drive_file_url && <a className="mt-1 inline-block text-cyan-300 underline" href={item.drive_file_url} target="_blank" rel="noreferrer">Open in Google Drive</a>}</div>)}</div></section>
      </div>
    </AdminShell>
  );
}
